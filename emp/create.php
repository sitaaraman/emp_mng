...
if ($id != "" && !isset($_POST['country_id'])) {
    $emp = $conn->query("SELECT * FROM employee WHERE id=$id")->fetch_assoc();
}

// Preserve submitted values when country is changed
...
if (isset($_POST['save'])) {
    ...
    if ($_POST['id'] == "") {
        $conn->query("INSERT INTO employee(first_name,last_name,email,salary,address,city,state_id,country_id) VALUES('$fn','$ln','$eml','$sa','$ad','$ci','$st','$co')");
    } else {
